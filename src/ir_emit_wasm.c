...
/* ============================================================
 * L10: vdag.sourcemap + minimal DWARF custom sections
 *
 * The wasm backend records a (fn_name, line, col) tuple per function
 * during its pre-pass and emits two custom sections:
 *
 *   `vdag.sourcemap` -- JSON the Visual-DAG runtime reads at instantiate
 *   time to translate a wazero trap (funcidx + byte offset) into a
 *   Limceron (file, line, col) triple at function-level granularity.
 *
 *   `.debug_info` + `.debug_line` -- minimal DWARF-shaped payloads
 *   sufficient for tools like wasmtime's `--debug` flag to surface a
 *   file/line in stack frames.
 * ============================================================ */

#define WASM_MAX_SRCMAP 256
typedef struct { const char *fn_name; const char *file; uint32_t line; uint32_t col; } LcnSrcMap;
static LcnSrcMap g_srcmap[WASM_MAX_SRCMAP];
static int g_srcmap_count = 0;
static const char *g_srcmap_file = NULL;

static const char *srcmap_basename(const char *path) {
    if (!path) return "<unknown>";
    const char *slash = strrchr(path, '/');
    return slash ? slash + 1 : path;
}

static void scan_sourcemap(EmitCtx *ctx, AstNode *program, const char *input) {
    g_srcmap_count = 0;
    g_srcmap_file = srcmap_basename(input);
    if (!program || program->kind != AST_PROGRAM) return;
    AstNode *decl;
    for (decl = program->params; decl; decl = decl->next) {
        if (decl->kind != AST_FN || !decl->name) continue;
        if (g_srcmap_count >= WASM_MAX_SRCMAP) break;
        IrFunction *fn;
        for (fn = ctx->module->functions; fn; fn = fn->next) {
            if (!fn->name) continue;
            const char *uname = user_name_for(fn->name);
            if (strcmp(uname, decl->name) != 0) continue;
            g_srcmap[g_srcmap_count].fn_name = fn->name;
            g_srcmap[g_srcmap_count].file    = g_srcmap_file;
            g_srcmap[g_srcmap_count].line    = decl->loc.line;
            g_srcmap[g_srcmap_count].col     = decl->loc.column;
            g_srcmap_count++;
            break;
        }
    }
...
static void emit_sourcemap_custom_section(EmitCtx *ctx) {
    FILE *out = ctx->out;
    if (g_srcmap_count <= 0) return;
    fprintf(out,
        "  ;; --- L10: source-map (DAG <-> Limceron <-> WAT chain) ---\n"
        "  ;; %d function(s)\n"
        "  (@custom \"vdag.sourcemap\" \"{\\\"file\\\":\\\"%s\\\",\\\"fns\\\":[",
        g_srcmap_count, g_srcmap_file ? g_srcmap_file : "<unknown>");
    int i;
    for (i = 0; i < g_srcmap_count; i++) {
        fprintf(out,
                "%s{\\\"name\\\":\\\"%s\\\",\\\"line\\\":%u,\\\"col\\\":%u}",
                i == 0 ? "" : ",",
                g_srcmap[i].fn_name, g_srcmap[i].line, g_srcmap[i].col);
    }
    fprintf(out, "]}\")\n");
}

static void emit_dwarf_custom_sections(EmitCtx *ctx) {
    FILE *out = ctx->out;
    if (g_srcmap_count <= 0) return;
    const char *file = g_srcmap_file ? g_srcmap_file : "<unknown>";
    fprintf(out,
        "  ;; --- L10: minimal DWARF .debug_info ---\n"
        "  (@custom \".debug_info\" "
        "\"{\\\"producer\\\":\\\"limceron-stage0 \" \"" LCN_VERSION "\\\","
        "\\\"language\\\":\\\"limceron\\\",\\\"comp_dir\\\":\\\".\\\","
        "\\\"name\\\":\\\"%s\\\"}\")\n",
        file);
    fprintf(out,
        "  ;; --- L10: minimal DWARF .debug_line ---\n"
        "  (@custom \".debug_line\" \"{\\\"files\\\":[\\\"%s\\\"],\\\"rows\\\":[",
        file);
    int i;
    for (i = 0; i < g_srcmap_count; i++) {
        fprintf(out,
                "%s{\\\"fn\\\":\\\"%s\\\",\\\"file\\\":0,\\\"line\\\":%u,\\\"col\\\":%u}",
                i == 0 ? "" : ",",
                g_srcmap[i].fn_name, g_srcmap[i].line, g_srcmap[i].col);
    }
    fprintf(out, "]}\")\n");
...
static void emit_module(FILE *out, IrModule *mod, EmitCtx *ctx) {
    ...
    emit_sourcemap_custom_section(ctx);
    emit_dwarf_custom_sections(ctx);
...
int lcn_emit_wasm(AstNode *program, const char *input,
    ...
    scan_sourcemap(&ctx, program, input);
...
int lcn_emit_wasm_wat(AstNode *program, FILE *out, Arena *arena,
    ...
    scan_budget(&ctx, program);
    scan_capability_network_allowlist(&ctx, program);
    scan_sourcemap(&ctx, program, "<test>");
    ctx.data_total  = ctx.data_offset;
